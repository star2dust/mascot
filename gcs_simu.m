...
mref = struct('pr',pr,'pr_dot',zeros(size(pr)),'p_lb',p_lb,'p_ub',p_ub,...
    ...
    'lam_pc',lam_pc,'lam_pc_dot',zeros(size(pr)),'f_curr',formation,'f_prev',formation,'color',cl,...    
...
% end
h = plot(gcs.SimuAxes,sum(ref.pr(1,:))/size(ref.pr,2),sum(ref.pr(2,:))/size(ref.pr,2),[ref.color(end) 'p']);
if ref.color(end)=='y'
    set(h,'Color',[1 0.5 0]);
end
end
